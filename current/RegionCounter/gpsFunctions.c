...
int traceStartTime[NUMBER_OF_EMULATOR_CLIENTS];						// read this from the first line of the file when you open the file for the very first time
...
int main(int argc, char** argv) {
	...
	if(argc < 3 ) {
		printf("Usage : Entere number of nodes and trace folder \n");
		exit(1);
	}
...
//		printf("Return value is %d \n",retValue);
		if(retValue!=-1 && retValue != -2 ) {
			printf("Time Period is: %d, node ID is %d , gps Coordinates are : " ,timeInstant,i);printCoordinates(retrievedCoordinate);
		}
	     }
		timeInstant=timeInstant+1;
	}
	while(timeInstant<=6000) ;	
}
